...
12. **`<Clarification>` is unauthorable — repair the shape and un-vacate the approve gate**
    ...
    all, and `RM-AGENT-RELIABILITY` D12's approve gate — which reads `node.attributes.target` at
