...
|---|---|---|---|---|---|
| P0 | Reject, don't filter: the integrity cluster | RC-1 | 6.2.0 | steps | `EXECUTING` — not complete. Delivered by `C-TOKEN-INTEGRITY`: P0.1, P0.2, P0.3, P0.5. Delivered by `C-CURSOR-INTEGRITY` (T-001–T-008 executed; authority close pending): P0.4 (epoch bounds), P0.6 (orphan inventory + `cursor recover` diagnose/`--fix` + single-controller auto-open), P0.10 (`review.attempt-pair-unsubstantiated`), D8.8 (status open vs folded epochs), F10 (catalog namespace + `makeFinding` visibility). Delivered by `C-RECOVER-FOLDABLE` (T-001–T-003 executed; authority close pending): F13 fix — `recover --fix` append-only effective-range supersession (LWW per worker), covering ceiling `max(requirement, openedId+98)`, damaged-shape repair, clean no-preseeded-terminal E2E; unblocks fold for `C-TOKEN-INTEGRITY` and completes the fold half of `AC-RECOVER-FIX-PRESERVES-ORPHAN`. P0.8 (calibration) remains out of this bundle (D8.1) — not yet authored. Assigned to `C-REPORT-HONESTY`: P0.7, P0.9. Derivation: [p0-derivation.md](./p0-derivation.md), [p0-cursor-derivation.md](./p0-cursor-derivation.md) |
