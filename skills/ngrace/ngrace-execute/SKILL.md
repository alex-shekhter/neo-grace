...
9. Before setting `applied` or archiving, run `ngrace gate apply --change C-ID` and `ngrace gate archive --change C-ID`. Each evaluates and records a Decision in `run-ledger.xml`; exit non-zero means refuse. Do not set status or move the bundle when refused. The gate does not itself author `status` or archive paths.
