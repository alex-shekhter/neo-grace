...
| 4 | Non-empty machine-checkable baseline and target assertions. `BaselineAssertions` assert only what the bundle leaves unchanged: a needle the bundle flips by design belongs in a test or a probe, not in a baseline assertion. |
| 5 | Explicit durable and observed write scopes (or `<None />` when there are no writes). |
| 6 | Scope covers what the deliverable forces, not only the files it targets: a skill-text change declares the skill-footprint pin; a rule change declares the fixtures that construct that rule. If the approved deliverable makes an edit inevitable, list that path at plan time — never leave the executor choosing between a scope breach and a failed task. Failure shape: `review.write-evidence-outside-scope`. A module-ownable `src/` path in `ObservedWriteScope` that does not exist yet warns `change.graph-anchors-pending-file` until the task that creates it lands, and the error `change.graph-anchors-miss-write-scope` applies once the file exists without an owning `LINKS:`; the plan is still approved, the approval commit names the predicted warning, and the executor baseline exit reflects it. Scope **the close writes** too, not only what the tasks write: when the approved spec assigns an act to a party other than the executor, that act's write targets still belong in the plan's `ObservedWriteScope` or the close's scope audit errors. Read the write set from the verb, never from memory: the flush writes the record documents it annotates while the bundle is active, the retirement move writes the retired siblings and rewrites the live roots, and the close adds the bundle's own archive arrival. Before approving the plan, confirm every act the spec assigns away from the executor has its write targets in `ObservedWriteScope` — the approval-time check that catches a scope omitting the close's writes. |
...
| 15 | Before the plan is approved, after a sufficient lexicon phrase, run `ngrace review --path . --change C-ID` (does not record a verdict), then `ngrace gate approve --change C-ID`. Refuse means unresolved Clarifications on IC-* / INV-* unless force; the gate writes status and records the fingerprint. Request ratification of C-ID at plan stage with a sufficient phrase: `approved`, `I approve`, or `approve this plan`. The approval write may belong to a party other than the author of the artifact: whoever holds the approving phrase writes it, and the author requests ratification without collapsing the two roles. At spec-approve the scope and WriteEvidence audits report they did not run (no plan); attempt-pair reports ran over 0 pairs and that is not substantiation. At plan-approve the review skips the bundle's own `spec.xml`, `plan.xml` and `design-context.xml` by identity; `decisions.md` is reported and is expected only when that file changed. Do not add those paths to `ObservedWriteScope`. The entries are structurally redundant rather than merely disfavored: the review already skips the bundle's own `spec.xml`, `plan.xml` and `design-context.xml` by identity, and reports `decisions.md` as expected rather than as a breach. |
...
| 19 | Drive every planned command on a prototype before the plan is approved: a `Verification` command that has never been executed against a fixture is a claim, and a gate it cannot satisfy is discovered by the executor mid-flight instead of by you now. Probes mutate copies, never the production record. Also drive the spec's `CloseEvidence` commands and the full CI in a faithful post-close copy at plan time, not only against the pre-close tree; a red-direction fixture is state-dependent exactly as a green assertion is: it locates its target by walking both layers and mutating the holder it finds, never a file named in advance. Drive each planned `Verification` command in the tree state its task leaves, never at HEAD, where a per-task cumulative `git diff` is trivially green; and scope a per-task byte-invariance command so it excludes every earlier task's files, or to the paths the task itself must not touch rather than everything-but-mine. A baseline `MustContain` / `MustNotContain` needle is evaluated by nothing before the plan is approved: drive every one with `--assertions baseline` on an approved-shaped copy, and bound a `MustNotContain` on a spelling other functions share to a test slice, never a whole file. Drive `--assertions target` / `--assertions final` and `--run-commands` on an approved-shaped copy (`ngrace gate approve --artifact plan` on the copy, then `ngrace cursor advance --task T-NNN --open-epoch`), never against the production bundle. A baseline drive whose `BaselineAssertions` declare `MustPassCommand`s carries `--run-commands` inside an open epoch on an approved-shaped copy too; without it the run reports `assertion.command-not-evaluated` for every declared command, a report that reads like breakage and is not. The faithful post-close copy carries `.ngrace/scratch` read-only, because without it `review.confidently-wrong` fires on archived plans whose `BaselineAssertions` `MustExist` the buffer path and a planner reads defects the production tree does not show. A faithful post-close copy also carries `.git`, because every `git diff --exit-code` `CloseEvidence` command needs it to evaluate, and an ignored directory reached through a symlink needs a bare entry in the copy's `.git/info/exclude` — a trailing-slash ignore pattern matches no symlink. On a copy, commit the approved plan before opening the epoch: an uncommitted `plan.xml` enters every attempt's `WriteEvidence`, and after the close's archive move the WriteEvidence scope audit reddens on it. On the post-close copy, `ngrace gate verdict --change C --outcome pass` (acking the post-archive unevaluated findings) evaluates every `CloseEvidence` command through the shell, refuses `pass` on a non-zero exit, and records `Result` and `Exit` per criterion on the verdict — read them from the ledger, never from a hand-rolled reader. |
| 20 | A task's `Verification` names the **narrowest instrument** that exercises that task's subject — the whole suite is not a per-task command. The full CI-equivalent verification is **close-time only**: it runs at the plan's `TargetAssertions` `MustPassCommand` under `--assertions final --run-commands`, and a `TargetAssertions` command does not duplicate what `final` will run. |
</must_do>
...
- If a plan deliberately omits a spec-affected anchor, declare it under optional `<OutOfPlanScope>` with a non-empty `<Reason>`:
  ...
  ```
- When the spec declares `AC-*` criteria, each task that implements an ordinary criterion without `CloseEvidence` should list it under optional `<Satisfies><AC-ID /></Satisfies>`. Unmapped criteria warn; Satisfies of unknown `AC-*` ids error.
...
- CloseEvidence `AC-*` are not Satisfies targets. `plan new` links only ordinary criteria. Satisfies of a complete CloseEvidence `AC-*` is an error. `Satisfies` maps only criteria without `CloseEvidence`; a close-bound `AC-*` is never a `Satisfies` target.
...
- Recommend `ngrace status --path PROJECT --json` after approval.
 - `--assertions target` and `--assertions final` require an approved plan, and `--run-commands` does not require an open epoch: it resolves a declared task in scope from an explicit task, a loose event, `run.xml`, or the last folded ledger event. A selected-assertion run — `--assertions baseline`, `target` or `final` — names its `--change C-*` identifier; without it the CLI refuses `assertion.change-required`.
</validation>
