...
export async function runPackedCliSmoke(repoRoot: string): Promise<void> {
  ...
  try {
    ...
    const cases: PackedSmokeCase[] = [
      ...
      { name: "lint undeclared flag", args: ["lint", "--path", project, "--format", "json", "--not-a-real-flag"], expectedExitCode: 1, assertStdout: (out) => { const value = jsonObject(out); const error = value.error as { code?: string; message?: string } | undefined; if (value.ok !== false || error?.code !== "invalid-arguments" || !error.message?.includes("--not-a-real-flag") || /\u001B/.test(error.message)) throw new Error("lint undeclared flag did not refuse with a clean invalid-arguments envelope"); } },
