...
Lineage path: `ngrace spec new --supersedes &lt;C-PRED&gt;` mints the successor and requires exactly one of a bare slug or `--supersedes`; it mints a throwaway skeleton — the successor is authored by overwriting the minted skeleton from the archived approved predecessor's bytes with `status` reset to `draft` and the verb-inserted `Replacement` child dropped, then applying a bounded diff — the id where it is data, the lineage paragraph in `Problem`, and the change that forced the supersede — and counting the diff and reporting the predecessor-id occurrences that remain. The `mint-search` line printed at mint, while the predecessor is still active and counted, is the value carried verbatim by whatever consumes it later; never re-derive it after the supersede moves the predecessor. `ngrace supersede` stamps `superseded` and `&lt;Replacement&gt;` on the spec and the draft plan and moves the bundle.
</shape_sources>
...
- An archive-edit criterion also excludes the predecessor's archive arrival after a supersede, with its reason: the supersede moves the predecessor into the archive after the successor's base commit, so without that exclusion the criterion is red by construction at every close.
...
- Before writing a byte-invariance pathspec, ask **what must the close write, and does every criterion permit it?** It is derivable mechanically: a non-empty `Pays` fixes the close's write set — the record documents the payment writes, any fixture that pins the payment relation, and the bundle's own archive arrival — and every one of them must be declared in the write scope and permitted by every byte-invariance pathspec **before** the pathspecs are written. A criterion that forbids the write the spec's own `Pays` requires can never be satisfied.
