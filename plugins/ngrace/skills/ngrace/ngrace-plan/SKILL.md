...
- Recommend `ngrace status --path PROJECT --json` after approval.
 - `--assertions target` and `--assertions final` require an approved plan, and `--run-commands` requires an open epoch on a declared task. A selected-assertion run — `--assertions baseline`, `target` or `final` — names its `--change C-*` identifier; without it the CLI refuses `assertion.change-required`.
</validation>
