...
5. After each write-plus-verification cycle, record the attempt (`ngrace cursor attempt --outcome pass|fail`). On the second failed attempt the task pauses for replan (budget of 2) — escalate rather than keep fixing.
