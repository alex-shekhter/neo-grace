...
>
> The CLI is what validates `.grace` artifacts — XML well-formedness, required sections,
...
6. Print created files and recommend the next workflow: use `ngrace-spec` to create an active `GraceChangeSpec`, then `ngrace-plan` to produce a `GraceChangePlan` before implementation.
</steps>
