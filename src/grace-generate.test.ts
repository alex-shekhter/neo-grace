import { closeSync, existsSync, fstatSync, lstatSync, mkdirSync, mkdtempSync, openSync, readFileSync, readdirSync, realpathSync, rmdirSync, rmSync, statSync, symlinkSync, writeFileSync } from "node:fs";
...
function prepinFixtureDir(): string {
  return path.join(repoRoot, "scripts/fixtures", `prepin-${PREPIN_COMMIT}`);
}
...
describe("pin identity and release (C-LINUX-VALIDATION-REPAIR-1-DE5A1A05)", () => {
  ...
  it.skipIf(!FS_RECYCLES)("AC-CLEANUP-BOUNDED pre-pin control: b53d13d deletes the recycled replacement", () => {
    const extracted = mkdtempSync(path.join(tmpdir(), "pin-baseline-"));
    const loaded = loadPrepinFixture(extracted);
    expect(loaded.problems, loaded.problems.join("\n")).toEqual([]);
    const entry = path.join(extracted, "src", "grace-generate.ts");
    expect(importContainmentProblem(extracted, entry, "cleanupCandidate")).toBeNull();
    try {
...
describe("prepin fixture bytes", () => {
  it("prepin fixture bytes: an escaping member fails closed and writes nothing outside", () => {
    const dest = mkdtempSync(path.join(tmpdir(), "prepin-escape-"));
    const outside = path.resolve(dest, "..", "prepin-escape-outside.txt");
    const header = Buffer.alloc(512);
    const payload = Buffer.from("x");
    Buffer.from("../prepin-escape-outside.txt").copy(header);
    header.write(payload.length.toString(8).padStart(11, "0"), 124);
    header[156] = "0".charCodeAt(0);
    const body = Buffer.alloc(512);
    payload.copy(body);
    const problems = extractPrepinArchive(Buffer.concat([header, body, Buffer.alloc(1024)]), dest);
    expect(problems.some((problem) => problem.includes("fails closed"))).toBe(true);
    expect(existsSync(outside)).toBe(false);
    rmSync(dest, { recursive: true, force: true });
  });

  it("prepin fixture bytes: extraction reconstructs the pinned src tree", async () => {
    const dest = mkdtempSync(path.join(tmpdir(), "prepin-bytes-"));
    try {
      const loaded = loadPrepinFixture(dest);
      expect(loaded.problems, loaded.problems.join("\n")).toEqual([]);
      expect(readdirSync(prepinFixtureDir()).sort()).toEqual(["archive.tar", "manifest.json"]);
      expect(srcTreeId(path.join(dest, "src"))).toBe(PREPIN_SRC_TREE);
      const entry = path.join(dest, "src", "grace-generate.ts");
      expect(importContainmentProblem(dest, entry, "cleanupCandidate")).toBeNull();
      const outside = importContainmentProblem(dest, path.join(repoRoot, "src", "grace-generate.ts"), "cleanupCandidate");
      expect(outside).toContain("fails closed");
      symlinkSync(path.join(repoRoot, "node_modules"), path.join(dest, "node_modules"), process.platform === "win32" ? "junction" : "dir");
      const imported = await import(entry);
      expect(typeof imported.cleanupCandidate).toBe("function");

      const target = path.join(dest, "src", "grace-generate.ts");
      const original = readFileSync(target);
      const flipped = Buffer.from(original);
      flipped[0] ^= 0xff;
      writeFileSync(target, flipped);
      expect(reconstructionProblems(dest, loaded.manifest).some((problem) => problem.includes("fails closed"))).toBe(true);
      writeFileSync(target, original);
      expect(reconstructionProblems(dest, loaded.manifest)).toEqual([]);

      rmSync(target);
      expect(reconstructionProblems(dest, loaded.manifest).some((problem) => problem.includes("missing extracted file"))).toBe(true);
      writeFileSync(target, original);
      expect(reconstructionProblems(dest, loaded.manifest)).toEqual([]);

      const extra = path.join(dest, "src", "prepin-extra.ts");
      writeFileSync(extra, "export const extra = 1;\n");
      expect(reconstructionProblems(dest, loaded.manifest).some((problem) => problem.includes("extra extracted file"))).toBe(true);
      rmSync(extra);
      expect(reconstructionProblems(dest, loaded.manifest)).toEqual([]);

      const paddedDest = mkdtempSync(path.join(tmpdir(), "prepin-pad-"));
      try {
        const padded = Buffer.concat([readFileSync(path.join(prepinFixtureDir(), "archive.tar")), Buffer.from("X")]);
        expect(extractPrepinArchive(padded, paddedDest)).toEqual([]);
        expect(reconstructionProblems(paddedDest, loaded.manifest)).toEqual([]);
      } finally {
        rmSync(paddedDest, { recursive: true, force: true });
      }

      const cross = prepinCommitCrossCheck(repoRoot, loaded.manifest);
      if (cross.reason === null) {
        expect(cross.tree).toBe(PREPIN_SRC_TREE);
        expect(cross.blobMismatches).toEqual([]);
      } else {
        expect(cross.reason).toBe(PREPIN_COMMIT_ABSENT_REASON);
      }
      const empty = mkdtempSync(path.join(tmpdir(), "prepin-absent-"));
      try {
        expect(gitText(["init", "-q"], empty).status).toBe(0);
        const absent = prepinCommitCrossCheck(empty, loaded.manifest);
        expect(absent.reason).toBe(PREPIN_COMMIT_ABSENT_REASON);
        expect(srcTreeId(path.join(dest, "src"))).toBe(PREPIN_SRC_TREE);
      } finally {
