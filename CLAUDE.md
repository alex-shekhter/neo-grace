...
- For CLI changes, run `bun run validate:cli` and exercise `ngrace lint` against a complete temporary or fixture neo-grace project. This packaging repository hosts a thin `.ngrace` tree for dogfooding; `bun run ngrace lint --path .` is expected to pass. That green result depends on `.ngrace-lint.json` (`ignoredDirs: ["examples"]`): `examples/` is a nested project covered by `validate:examples`, and `scripts/` is adopted — brought under the root `typecheck` and the root lint, with every one of the 10 typecheck and 36 lint errors repaired and `M-RELEASE-AUTOMATION` added to the graph — by `C-SCRIPTS-ADOPTION-2-36DEB1BD`, which pays the earlier deferral. Before that bundle the root lint reported 36 real errors under `scripts/`; it now reports none.
